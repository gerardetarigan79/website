...
if (oldBlock.test(source)) {
  source = source.replace(oldBlock, newBlock);
  fs.writeFileSync(path, source);
  console.log("logo-loop-fix: Adobe Creative Cloud + Cinema 4D installed; Element 3D removed");
} else if (source.includes('logo("adobecreativecloud","Adobe Creative Cloud"') && source.includes('logo("cinema4d","Cinema 4D"')) {
  console.log("logo-loop-fix: already applied");
} else {
  throw new Error("logo-loop-fix: could not find skillLogoGroups block");
}
