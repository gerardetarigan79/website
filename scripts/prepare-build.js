...
const projectsReplacement = `function Projects(){const projects=[{name:"Doxa Dock",subtitle:"Discord user automation toolkit",description:"Lightweight client utility and automation dashboard I built to give me deeper control over account workflows. It streamlines daily tasks with custom scripts, real-time activity logging, and tailored profile.",logo:"/projects/doxa.png",className:"doxa"},{name:"Vanta Flow",subtitle:"After Effects workflow plugin",description:"An After Effects plugin built around essential tools, shortcuts, presets, and workflow utilities to make editing faster and more efficient.",logo:"/projects/vanta.png",className:"vanta"},{name:"Argo Node",subtitle:"Discord utility bot",description:"Multipurpose Discord user application that brings smart media utilities, social lookups, and AI tools directly into any chat. Built on Discord's official User App framework, it allows power users to execute commands seamlessly across servers, group chats, and direct messages.",logo:"/projects/argo.png",className:"argo"}];return <Page id="projects" kicker="my work" title="projects"><p className="lead">projects i build to improve my workflow.</p><div className="projects-grid">{projects.map(project=><div className={\`project-card project-card-\${project.className} cursor-target\`} key={project.name}><div className="project-logo-wrap"><img className="project-logo" src={project.logo} alt={project.name}/></div><div className="project-body"><div className="project-brand"><span>{project.name}</span></div><h2>{project.subtitle}</h2><p>{project.description}</p></div></div>)}</div></Page>}`;
