...
const LightRays = ({ raysOrigin = 'top-center', raysColor = '#ffffff', raysSpeed = 1, lightSpread = 1, rayLength = 2, pulsating = false, fadeDistance = 1, saturation = 1, followMouse = true, mouseInfluence = 0.1, noiseAmount = 0, distortion = 0, className = '' }) => {
  const containerRef = useRef(null), uniformsRef = useRef(null), rendererRef = useRef(null), mouseRef = useRef({ x: 0.5, y: 0.5 }), smoothMouseRef = useRef({ x: 0.5, y: 0.5 }), animationIdRef = useRef(null), meshRef = useRef(null), cleanupRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const mesh = new Mesh(gl, { geometry:new Triangle(gl), program:new Program(gl,{vertex:vert,fragment:frag,uniforms}) }); meshRef.current=mesh;
      ...
      cleanupRef.current=()=>{cancelAnimationFrame(animationIdRef.current);window.removeEventListener('resize',resize);window.removeEventListener('mousemove',mouse);renderer.gl.getExtension('WEBGL_lose_context')?.loseContext();rendererRef.current=null;uniformsRef.current=null;meshRef.current=null;};
