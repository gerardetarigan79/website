...
function install() {
  ...
  const style = document.createElement("style"); style.textContent = `
    #${CONTROL_ID}{position:fixed;right:12px;top:12px;z-index:60;height:32px;width:32px;padding:0;display:flex;align-items:center;justify-content:center;box-sizing:border-box;border:1px solid #24242a;border-radius:7px;background:#101014;color:#777;box-shadow:0 8px 25px #0006;opacity:0;visibility:hidden;transform:translateY(-12px) scale(.96);transition:width .38s cubic-bezier(.2,.75,.2,1),opacity .55s cubic-bezier(.2,.75,.2,1),transform .7s cubic-bezier(.2,.75,.2,1),visibility 0s linear .7s}
    #${CONTROL_ID}.visible{opacity:1;visibility:visible;transform:none;transition-delay:0s}
    #${CONTROL_ID}.expanded{width:120px}
    #${CONTROL_ID} .draven-audio-mute{width:22px;height:22px;padding:0;border:0;background:transparent;color:#777;display:grid;place-items:center;cursor:none;flex:0 0 22px;transition:color .18s,transform .18s}
    ...
    #${CONTROL_ID} .draven-audio-slider-wrap{width:0;overflow:hidden;opacity:0;transform:translateX(5px);transition:width .34s cubic-bezier(.2,.75,.2,1),opacity .24s ease,transform .34s cubic-bezier(.2,.75,.2,1)}
    #${CONTROL_ID}.expanded .draven-audio-slider-wrap{width:72px;opacity:1;transform:none;margin-left:7px}
    #${CONTROL_ID} .draven-audio-slider{display:block;width:72px;height:18px;margin:0;padding:0;background:transparent;appearance:none;-webkit-appearance:none;cursor:none}
    ...
    @media(max-width:500px){#${CONTROL_ID}{right:10px;top:10px}#${CONTROL_ID}.expanded{width:104px}#${CONTROL_ID}.expanded .draven-audio-slider-wrap{width:58px}#${CONTROL_ID} .draven-audio-slider{width:58px}}
