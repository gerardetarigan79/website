...
function installBackgroundAudio(){
  ...
  const makeIcon=muted=>muted
    ?`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5 6 9H3v6h3l5 4V5Z"/><path d="m19 9-5 6m0-6 5 6"/></svg>`
    :`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M11 5 6 9H3v6h3l5 4V5Z"/><path d="M15.5 8.5a5 5 0 0 1 0 7M18 6a8.5 8.5 0 0 1 0 12"/></svg>`;

  const control=document.createElement("div");
  control.className="draven-audio-control";
  control.setAttribute("aria-label","Background music controls");
  control.innerHTML=`<button class="draven-audio-mute" type="button" aria-label="Mute background music" title="Mute background music">${makeIcon(audio.muted)}</button><input class="draven-audio-slider" type="range" min="0" max="1" step="0.01" value="${volume}" aria-label="Background music volume" title="Background music volume">`;
  document.body.appendChild(control);

  const muteButton=control.querySelector(".draven-audio-mute");
  const slider=control.querySelector(".draven-audio-slider");

  const save=()=>{
    localStorage.setItem(BG_AUDIO_VOLUME_KEY,String(audio.volume));
    localStorage.setItem(BG_AUDIO_MUTED_KEY,audio.muted?"1":"0");
  };
  const refreshMute=()=>{
    const muted=audio.muted||audio.volume===0;
    muteButton.innerHTML=makeIcon(muted);
    muteButton.setAttribute("aria-label",muted?"Unmute background music":"Mute background music");
    muteButton.title=muted?"Unmute background music":"Mute background music";
  };
  ...
  muteButton.addEventListener("click",()=>{
    if(audio.muted||audio.volume===0){
      const restore=Number.parseFloat(localStorage.getItem(BG_AUDIO_VOLUME_KEY));
      const next=Number.isFinite(restore)&&restore>0?restore:0.35;
      audio.volume=next;
...
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",installBackgroundAudio,{once:true});
else installBackgroundAudio();
