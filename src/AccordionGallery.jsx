...
const AccordionGallery = ({ items = DEFAULT_ITEMS, defaultIndex = 2, accentColor = '#ffffff', overlayColor = '#060010', textColor = '#ffffff', height = 460, gap = 10, radius = 16, expandRatio = 0.52, orientation = 'horizontal', duration = 0.6, ease = 'power3.out', parallax = 0.5, tilt = 8, stagger = 0.06, trigger = 'hover', showLabels = true, grayscale = true, className = '' }) => {
  const rootRef = useRef(null), panelRefs = useRef([]), mediaRefs = useRef([]), barRefs = useRef([]), textRefs = useRef([]), firstRunRef = useRef(true), mediaSizeRef = useRef(320);
  ...
  const applyLayout = useCallback(animate => {
    ...
    panels.forEach((panel, i) => {
      ...
      const media = mediaRefs.current[i], bar = barRefs.current[i], text = textRefs.current[i];
      const rot = isActive ? 0 : i < active ? tilt : -tilt;
      const rotProp = vertical ? { rotateX: -rot } : { rotateY: rot };
      gsap.to(panel, { flexGrow: isActive ? grow : 1, ...rotProp, duration: dur, ease });
      if (media) {
      ...
      if (showLabels && bar && text) {
        ...
        gsap.set(bar, { opacity: isActive ? 1 : 0 });
      }
    });
  }, [active, count, expandRatio, duration, ease, vertical, tilt, parallax, grayscale, showLabels, prefersReduced]);
  ...
  return <div ref={rootRef} className={`accordion-gallery${vertical ? ' accordion-gallery--vertical' : ''}${className ? ` ${className}` : ''}`} style={{ '--ag-accent': accentColor, '--ag-overlay': overlayColor, '--ag-text': textColor, '--ag-gap': `${gap}px`, '--ag-radius': `${radius}px`, height: vertical ? `${Math.round(height * 1.6)}px` : `${height}px` }} role="list" aria-label="Image accordion gallery">
    {items.map((item, i) => {
      const isActive = i === active;
      ...
      return <Tag key={i} ref={el => (panelRefs.current[i] = el)} className={`ag-panel${isActive ? ' ag-panel--active' : ''}`} style={{ borderRadius: `${radius}px` }} href={item.link || undefined} onClick={e => handleClick(i, e)} onMouseEnter={() => handleEnter(i)} onFocus={() => setActive(i)} onKeyDown={e => handleKeyDown(i, e)} role="listitem" tabIndex={0} aria-current={isActive ? 'true' : undefined} aria-label={item.label}>
        ...
        {showLabels && <span className="ag-panel__label" aria-hidden="true"><span className="ag-panel__bar" ref={el => (barRefs.current[i] = el)} /><span className="ag-panel__text" ref={el => (textRefs.current[i] = el)}><strong className="ag-panel__title">{item.label}</strong><span className="ag-panel__details">{item.subtitle && <small>{item.subtitle}</small>}{item.description && <p>{item.description}</p>}</span></span></span>}
      </Tag>;
    })}
  </div>;
};
