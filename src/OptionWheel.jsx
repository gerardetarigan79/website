...
const OptionWheel = ({
  ...
  const pageSyncLockRef = useRef(0), wheelInteractionRef = useRef(0), pageSyncTimerRef = useRef(null);
  ...
  const runFrame = useCallback(now => {
    ...
    if (Math.abs(target - next) >= 0.001) rafRef.current = requestAnimationFrame(runFrame);
    else rafRef.current = null;
  }, []);

  // Never restart an already-running render loop. Wheel input can update the target
  // continuously; restarting the interpolation clock on every wheel event was the
  // source of the visible jitter.
  ...
  // Wheel interaction has its own momentum target, while the render loop above stays
  // alive continuously. Page synchronization is temporarily paused during wheel input
  // and for a short settling period afterward, so the two controllers never fight.
  useEffect(() => {
    const el = rootRef.current; if (!el) return;
    const onWheel = e => {
      e.preventDefault();
      const cfg = cfgRef.current, delta = e.deltaMode === 1 ? e.deltaY * 24 : e.deltaY;
      wheelInteractionRef.current = performance.now() + 500;
      ...
      const consume = () => {
        ...
        if (Math.abs(velocity) < 0.002) {
          ...
          wheelInteractionRef.current = performance.now() + 350;
          return;
        }
  ...
  useEffect(() => {
    ...
    const syncToPage = () => {
      ticking = false;
      if (performance.now() < pageSyncLockRef.current || performance.now() < wheelInteractionRef.current) return;
    ...
    const onScroll = () => {
      if (performance.now() < wheelInteractionRef.current) return;
      if (!ticking) { ticking = true; requestAnimationFrame(syncToPage); }
    };
    window.addEventListener('scroll', onScroll, { passive: true });
    window.addEventListener('resize', syncToPage);
    const initial = setTimeout(syncToPage, 0);
    return () => { window.removeEventListener('scroll', onScroll); window.removeEventListener('resize', syncToPage); clearTimeout(initial); if (pageSyncTimerRef.current) clearTimeout(pageSyncTimerRef.current); };
  }, [items, applyTarget]);
  ...
  useEffect(() => () => { if (rafRef.current != null) cancelAnimationFrame(rafRef.current); if (wheelFrameRef.current != null) cancelAnimationFrame(wheelFrameRef.current); if (pageSyncTimerRef.current) clearTimeout(pageSyncTimerRef.current); rafRef.current = null; wheelFrameRef.current = null; audioRef.current?.pause(); }, []);
