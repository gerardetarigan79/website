...
const OptionWheel = ({ items = DEFAULT_ITEMS, defaultSelected = 3, onChange, textColor = '#a6a6a6', activeColor = '#ffffff', side = 'left', fontSize = 3, spacing = 1.4, curve = 1, tilt = 6, blur = 2, fade = 0.25, minOpacity = 0.05, smoothing = 200, inset = 80, loop = false, draggable = true, soundUrl = '', soundVolume = 0.5, className = '' }) => {
  ...
  const runFrame = useCallback(now => {
    ...
    if (Math.abs(target - next) >= 0.001) rafRef.current = requestAnimationFrame(runFrame); else rafRef.current = null;
  }, []);

  const startLoop = useCallback(() => { if (rafRef.current != null) return; lastRef.current = performance.now(); rafRef.current = requestAnimationFrame(runFrame); }, [runFrame]);
  ...
  useEffect(() => {
    ...
    const syncToPage = () => {
      ...
      const center = window.innerHeight * 0.38; let best = 0, bestDistance = Infinity;
      items.forEach((item, index) => { const id = String(item).trim().toLowerCase().replace(/\s+/g, '-'); const section = document.getElementById(id); if (!section) return; const rect = section.getBoundingClientRect(), distance = Math.abs(rect.top - center); if (distance < bestDistance) { bestDistance = distance; best = index; } });
      if (best !== selectedRef.current) applyTarget(best, true, 'scroll');
    };
    const onScroll = () => { if (performance.now() < wheelInteractionRef.current) return; if (!ticking) { ticking = true; requestAnimationFrame(syncToPage); } };
    window.addEventListener('scroll', onScroll, { passive: true }); window.addEventListener('resize', syncToPage); const initial = setTimeout(syncToPage, 0);
