...
export default function RecentPlaysCarousel({tracks = []}) {
  ...
  return <div className={`recent-carousel${dragging ? " is-dragging" : ""}`} tabIndex={0} onKeyDown={onKeyDown} aria-label="Recent plays carousel">
    <div className="recent-carousel-track">
      {sequence.map((item, index) => {
        ...
        const style = {
          ...
          opacity: distance > 3 ? 0 : boundary && distance > 2 ? .25 : Math.max(.25, 1 - distance * .2), zIndex: 30 - distance
        };
        if (boundary) return <article className={`recent-card is-boundary${activeCard ? " is-active" : ""}`} style={style} key={item.key} aria-hidden={!activeCard}>
          <div className="lastfm-boundary-card"><div className="lastfm-logo" aria-label="Last.fm">last<span>.fm</span></div><small>{item.side === "start" ? "FIRST SCROBBLE" : "END OF SCROBBLES"}</small></div>
        </article>;

        return <article className={`recent-card${activeCard ? " is-active" : ""}`} style={{...style, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start", width: "280px", height: "auto"}} key={item.key}>
          <div className="recent-art-wrap recent-cd-drag-zone" style={{width: "280px", height: "280px", flex: "0 0 280px", position: "relative"}} onPointerDown={onCdPointerDown} onPointerMove={onCdPointerMove} onPointerUp={onCdPointerUp} onPointerCancel={onCdPointerCancel} onLostPointerCapture={onCdLostPointerCapture} title="Drag to browse recent plays">
            <div className="recent-cd" style={{backgroundImage: item.image ? `url(${item.image})` : "none"}} aria-hidden="true" />
            {!item.image && <div className="recent-cover-fallback">Last.fm</div>}
          </div>
          <div className="recent-card-info-row" style={{width: "100%", marginTop: "14px", textAlign: "center", flex: "0 0 auto"}}>
    ...
    <div className="recent-carousel-hint">drag the CD to explore</div>
  </div>;
}
