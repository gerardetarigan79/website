...
const useAnimationLoop = (trackRef, targetVelocity, seqWidth, seqHeight, hoverRef, hoverSpeed, isVertical) => {
  ...
  useEffect(() => {
    ...
    const animate = timestamp => {
      ...
      const target = hoverRef.current && hoverSpeed !== undefined ? hoverSpeed : targetVelocity;
...
export const LogoLoop = memo(({ logos, speed=120, direction='left', width='100%', logoHeight=28, gap=32, pauseOnHover, hoverSpeed, fadeOut=false, fadeOutColor, scaleOnHover=false, renderItem, ariaLabel='Partner logos', className, style }) => {
  ...
  const [seqWidth,setSeqWidth]=useState(0),[seqHeight,setSeqHeight]=useState(0),[copyCount,setCopyCount]=useState(ANIMATION_CONFIG.MIN_COPIES);
  ...
  const handlePointerEnter=useCallback(()=>{hoverRef.current=effectiveHoverSpeed!==undefined},[effectiveHoverSpeed]);
  const handlePointerLeave=useCallback(()=>{hoverRef.current=false},[]);
  ...
  return <div ref={containerRef} className={rootClassName} style={containerStyle} role="region" aria-label={ariaLabel} onPointerEnter={handlePointerEnter} onPointerLeave={handlePointerLeave}><div className="logoloop__track" ref={trackRef}>{logoLists}</div></div>;
});
