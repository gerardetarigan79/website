...
const useAnimationLoop = (trackRef, targetVelocity, seqWidth, seqHeight, hoverRef, hoverSpeed, isVertical) => {
  ...
  useEffect(() => {
    ...
    const animate = timestamp => {
      ...
      const targetMagnitude = hoverRef.current && hoverSpeed !== undefined ? hoverMagnitude : Math.abs(targetVelocity);
      ...
      if (seqSize > 0) {
        ...
        track.style.transform = isVertical ? `translate3d(0, ${-nextOffset}px, 0)` : `translate3d(${-nextOffset}px, 0, 0)`;
      }
...
export const LogoLoop = memo(({ logos, speed=120, direction='left', width='100%', logoHeight=28, gap=32, pauseOnHover, hoverSpeed, fadeOut=false, fadeOutColor, scaleOnHover=false, renderItem, ariaLabel='Partner logos', className, style }) => {
  ...
  useEffect(() => {const container=containerRef.current;if(!container||effectiveHoverSpeed===undefined)return;const updateHover=event=>{const rect=container.getBoundingClientRect();hoverRef.current=event.clientX>=rect.left&&event.clientX<=rect.right&&event.clientY>=rect.top&&event.clientY<=rect.bottom;};const clearHover=()=>{hoverRef.current=false;};window.addEventListener('pointermove',updateHover,{passive:true});window.addEventListener('pointerleave',clearHover,{passive:true});return()=>{window.removeEventListener('pointermove',updateHover);window.removeEventListener('pointerleave',clearHover);};},[effectiveHoverSpeed]);
  ...
  const renderLogoItem=useCallback((item,key)=>{if(renderItem)return <li className="logoloop__item" key={key} role="listitem">{renderItem(item,key)}</li>;const isNodeItem='node' in item;const isCreativeCloud=item.title==='Adobe Media Encoder';const logoProps=isCreativeCloud?{src:CREATIVE_CLOUD_LOGO,alt:'Adobe Creative Cloud',title:'Adobe Creative Cloud'}:{src:item.src,srcSet:item.srcSet,sizes:item.sizes,width:item.width,height:item.height,alt:item.alt??'',title:item.title};const visual=isNodeItem?<span className="logoloop__node" aria-hidden={!!item.href&&!item.ariaLabel}>{item.node}</span>:<span className="logoloop__visual"><img {...logoProps} loading="eager" decoding="async" draggable={false}/><img className="logoloop__reflection" {...logoProps} aria-hidden="true" alt="" loading="eager" decoding="async" draggable={false}/></span>;const itemAriaLabel=isNodeItem?(item.ariaLabel??item.title):(isCreativeCloud?'Adobe Creative Cloud':(item.alt??item.title));const itemContent=item.href?<a className="logoloop__link" href={item.href} aria-label={itemAriaLabel||'logo link'} target="_blank" rel="noreferrer noopener">{visual}</a>:visual;return <li className="logoloop__item" key={key} role="listitem">{itemContent}</li>;},[renderItem]);
