...
function FlipClock(){
  ...
  return <div className="widget flip-widget">
...
function AgeWidget(){
  ...
  return <div className="widget age-widget">
...
function WeatherWidget(){
  ...
  useEffect(()=>{
    fetch("https://api.open-meteo.com/v1/forecast?latitude=-7.2575&longitude=112.7521&current=temperature_2m,relative_humidity_2m,wind_speed_10m,weather_code&timezone=Asia%2FJakarta")
      .then(r=>r.json()).then(x=>setW(x.current)).catch(()=>setW(null)).finally(()=>setLoading(false));
  },[]);
  const code=w?.weather_code;
  const cloudy=code==null?"":code>=45?"Cloudy":code>=1?"Partly cloudy":"Clear";
  return <div className="widget weather-widget">
    <Cloud size={30}/>
    <div><strong>{loading?"Loading weather…":`It's currently ${Math.round(w?.temperature_2m||0)}°C, ${cloudy}`}</strong>
    <div className="weather-meta"><span><Wind size={11}/>{Math.round(w?.wind_speed_10m||0)} km/h</span><span><Droplets size={11}/>{w?.relative_humidity_2m||0}%</span></div></div>
...
function DiscordCard(){
  ...
  useEffect(()=>{
    const load=()=>fetch(`https://api.lanyard.rest/v1/users/${DISCORD_ID}`).then(r=>r.json()).then(x=>setP(x.data)).catch(()=>{}).finally(()=>setLoading(false));
  ...
  const decorationUrl=decoration?`https://cdn.discordapp.com/avatar-decoration-presets/${decoration}.png?size=128`:null;
  ...
  const badges=badgeDefs.filter(([bit])=>(flags&bit)===bit);
  const premium=user?.premium_type===2;
  return <div className="discord-card">
    <div className="avatar-wrap"><img className="avatar" src={avatar} /><span className={`presence ${p?.discord_status||"offline"}`}/>{decorationUrl&&<img className="avatar-decoration" src={decorationUrl}/>}</div>
...
function Home(){
  return <Page id="home" kicker="hey there" title="Draven">
    <p className="lead">i'm a <b>VFX and GFX designer, editor & Information Systems student</b> building things across creative software and modern web technology. check out my <a href="#projects">projects</a> or find me on <a href="#contact">discord</a>.</p>
    <DiscordCard/>
...
async function lastfm(type,extra=""){
  const r=await fetch(`/api/lastfm?type=${type}${extra}`); if(!r.ok)throw Error("Last.fm unavailable"); return r.json();
}
function Music(){
  const [data,setData]=useState({recent:[],artists:[],albums:[],info:null}),[loading,setLoading]=useState(true),[playing,setPlaying]=useState(false);
  const load=async()=>{
    try{
      const [recent,artists,albums,info]=await Promise.all([
        lastfm("recent"),lastfm("artists"),lastfm("albums"),lastfm("info")
      ]);
      setData({recent:recent.recenttracks?.track||[],artists:artists.topartists?.artist||[],albums:albums.topalbums?.album||[],info:info.user||null});
...
const skillGroups={
  "creative":[["After Effects",Video],["Photoshop",FileImage],["Adobe Media Encoder",Video],["Element 3D",Zap],["Figma",Palette]],
  "development":[["Visual Studio Code",Code2],["Git",GitBranch],["GitHub",Github],["Vercel",Globe2],["Java",Braces],["Lua",Terminal]],
  "databases & data":[["PostgreSQL",Database],["MySQL",Database],["SQLite",Database],["R",Code2],["RStudio",Code2],["GitHub Actions",Zap]]
};
function Projects(){return <Page id="projects" kicker="my work" title="projects"><p className="lead">commissions, collaborations, and personal builds. select work that might be the most.</p><div className="project-card"><div className="doxa-preview"><div className="doxa-title">Doxa Dock</div><div className="doxa-sub">private user application</div></div><div className="project-body"><div className="project-brand">D <span>doxa dock</span></div><h2>private user application</h2><p>a private application built with a focused interface and custom functionality.</p><a href="#contact">view project <ExternalLink size={10}/></a></div></div></Page>}
function Skills(){return <Page id="skills" kicker="my tech stack" title="skills"><p className="lead">a collection of software, frameworks, and tools i use to build amazing things.</p>{Object.entries(skillGroups).map(([g,x])=><div className="skill-group" key={g}><h3>{g}</h3><div className="pill-wrap">{x.map(([n,I])=><Pill key={n} icon={I}>{n}</Pill>)}</div></div>)}</Page>}
function Setup(){const hardware=[["CPU","Intel(R) Core(TM) i7-14650HX",Cpu],["GPU","NVIDIA GeForce RTX 5050",Monitor],["RAM","32 GB (2 x 16 GB) DDR5 5600MT/S",MemoryStick],["Storage","512 GB NVMe SSD",HardDrive],["Motherboard","ASUSTeK FX608JHR",Cpu],["Mouse","Glorious Model O Wireless",Mouse]];return <Page id="setup" kicker="what i use" title="setup"><p className="lead">the hardware, editor, and apps that show up in my workflow most days.</p><h3 className="group-title">hardware</h3><div className="setup-grid">{hardware.map(([a,b,I])=><div className="setup-card" key={a}><I size={17}/><div><strong>{b}</strong><small>{a}</small></div></div>)}</div></Page>}
function Games(){
  const [steam,setSteam]=useState(null);
  const [roblox,setRoblox]=useState("");
  useEffect(()=>{
    fetch("/api/steam").then(r=>r.json()).then(setSteam).catch(()=>setSteam({error:true}));
    fetch("/api/roblox").then(r=>r.json()).then(x=>setRoblox(x.imageUrl||"")).catch(()=>{});
  },[]);
  return <Page id="games" kicker="games & profiles" title="games"><p className="lead">a few places where i spend time when i'm not building things.</p>
...
function F1(){
  const [stand,setStand]=useState(null),[next,setNext]=useState(null),[updated,setUpdated]=useState(null);
...
function App(){
  ...
  useEffect(()=>{
    const sync=()=>{const hash=window.location.hash.replace("#","");if(PAGES.includes(hash))setActive(hash)};
    sync();
    const io=new IntersectionObserver(es=>{const visible=es.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(visible)setActive(visible.target.id)},{rootMargin:"-42% 0px -42% 0px",threshold:[0,.2,.5,1]});
