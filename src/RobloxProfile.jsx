...
function cdnUrl(hash) { if (!hash) return ""; let i = 31; for (let t = 0; t < Math.min(38, hash.length); t += 1) i ^= hash[t].charCodeAt(0); return `https://t${((i % 8) + 8) % 8}.rbxcdn.com/${hash}`; }
...
function RobloxAvatar3D({ model, fallbackUrl, loading }) {
  ...
  useEffect(() => {
    ...
    setFailed(false); const mount = mountRef.current; if (!mount) return undefined;
    let disposed = false; let frame; let avatar;
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera(25, 1, 0.01, 1000);
    camera.position.set(0, 1.72, 7.4); camera.lookAt(0, 1.72, 0);
    ...
    manager.setURLModifier((url) => { const hash = url.split("?")[0].split("/").pop(); return hash && /^[A-Za-z0-9_-]{16,}$/.test(hash) ? cdnUrl(hash) : url; });
    ...
    const load = async () => {
      try {
        ...
        Object.values(materials.materials || {}).forEach((material) => { material.transparent = false; material.alphaTest = 0.01; material.side = THREE.DoubleSide; material.needsUpdate = true; });
