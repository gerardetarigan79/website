...
const TargetCursor = ({targetSelector='.cursor-target',spinDuration=2,hideDefaultCursor=true,hoverDuration=.2,parallaxOn=true,cursorColor='#ffffff',cursorColorOnTarget}) => {
  ...
  useEffect(()=>{
    ...
    const createSpinTimeline=()=>{spinTl.current?.kill();spinTl.current=gsap.timeline({repeat:-1}).to(cursor,{rotation:'+=360',duration:spinDuration,ease:'none'});};createSpinTimeline();
    const tickerFn=()=>{if(!targetCornerPositionsRef.current||!cursorRef.current||!cornersRef.current)return;const strength=activeStrengthRef.current;if(strength===0)return;const cursorX=gsap.getProperty(cursorRef.current,'x'),cursorY=gsap.getProperty(cursorRef.current,'y');Array.from(cornersRef.current).forEach((corner,i)=>{const currentX=gsap.getProperty(corner,'x'),currentY=gsap.getProperty(corner,'y'),targetX=targetCornerPositionsRef.current[i].x-cursorX,targetY=targetCornerPositionsRef.current[i].y-cursorY,finalX=currentX+(targetX-currentX)*strength,finalY=currentY+(targetY-currentY)*strength,duration=strength>=.99?(parallaxOn?.2:0):.05;gsap.to(corner,{x:finalX,y:finalY,duration,ease:duration===0?'none':'power1.out',overwrite:'auto'});});};tickerFnRef.current=tickerFn;
