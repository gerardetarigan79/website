...
class CanvasTxt {
 ...
 render(){const {fontSize,fontFamily,color}=this.opts;this.context.clearRect(0,0,this.canvas.width,this.canvas.height);this.context.font=`${fontSize}px ${fontFamily}`;this.context.fillStyle=color;this.context.textBaseline='alphabetic';const m=this.context.measureText(this.txt);this.context.fillText(this.txt,10,10+m.actualBoundingBoxAscent);}
 get width(){return this.canvas.width;} get height(){return this.canvas.height;} get texture(){return this.canvas;}
}

export default function ASCIIText({text='David!',textFontSize=360,textColor='#fdf9f3',planeBaseHeight=14,enableWaves=true}){
 const containerRef=useRef(null);const instanceRef=useRef(null);
 useEffect(()=>{
  ...
  const init=async()=>{
   ...
   const renderer=new THREE.WebGLRenderer({antialias:false,alpha:true});renderer.setPixelRatio(1);renderer.setClearColor(0,0);container.replaceChildren(renderer.domElement);
   const textCanvas=new CanvasTxt(text,{fontSize:textFontSize,fontFamily:'IBM Plex Mono, monospace',color:textColor});textCanvas.resize();textCanvas.render();
   ...
   const geometry=new THREE.PlaneGeometry(planeBaseHeight*(textCanvas.width/textCanvas.height),planeBaseHeight,64,64);
   ...
   const camera=new THREE.PerspectiveCamera(45,width/height,1,1000);camera.position.z=30;renderer.setSize(width,height,false);
   const mouse={x:.5,y:.5};
   const onMove=e=>{const r=container.getBoundingClientRect();mouse.x=(e.clientX-r.left)/Math.max(1,r.width);mouse.y=1-(e.clientY-r.top)/Math.max(1,r.height);material.uniforms.uMouse.value.set(mouse.x,mouse.y);};
   const resize=()=>{const r=container.getBoundingClientRect();const w=Math.max(1,r.width),h=Math.max(1,r.height);renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();};
   const ro=new ResizeObserver(resize);ro.observe(container);container.addEventListener('mousemove',onMove);
   let frame;const loop=t=>{if(cancelled)return;material.uniforms.uTime.value=t*.001;mesh.rotation.y+=((mouse.x-.5)*.08-mesh.rotation.y)*.04;mesh.rotation.x+=((-(mouse.y-.5))*.06-mesh.rotation.x)*.04;renderer.render(scene,camera);frame=requestAnimationFrame(loop);};frame=requestAnimationFrame(loop);
  ...
  return()=>{cancelled=true;instanceRef.current?.dispose();instanceRef.current=null;};
